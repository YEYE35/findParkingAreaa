...
3. **검색 실행(버튼 클릭 또는 Enter)**
   
   ![설명](./imagesReadme/icon.png)
...
5. **결과 필터링**
   ![설명](./imagesReadme/filter.png)
   - 만약 Ai 추천 검색결과도 확인하고 싶다면 **결과 정렬 / 필터 버튼** 을 누르면 됩니다.
   - 그러면 각 주차장의 **기본 점수 + 피드백이 반영된 추천 점수**와 **good / bad 버튼**을 포함한 카드 UI가 뜨게 됩니다.
   
   ![설명](./imagesReadme/good.png)
   - 이제 버튼을 누르면, **추천 점수 가중치 로직**이 적용되어 점수가 올라가거나 내려갑니다.
---
## 라이선스
